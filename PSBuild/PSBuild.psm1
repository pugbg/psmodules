		...
		try
		{
			...
			if ($JobParams.ContainsKey('PSGetRepository'))
			{
				try
				{
					...
					{
						Assert-PSRepository -PSGetRepository $JobParams['PSGetRepository'] -ErrorAction Stop
...
function Build-PSModule
{
    ...
    Process
    {
		...
		{
			#Register PSGetRepo
			Assert-PSRepository -PSGetRepository $PSGetRepository -ErrorAction Stop
		}
					...
					try
					{
						...
						if ($ResolveDependancies.IsPresent)
						{
							...
							foreach ($ModDependency in $Dependancies)
							{
								...
								{
									Build-PSModule -SourcePath $ModuleValidationCache.Value[$dependantModuleName].ModuleInfo.ModuleBase -DestinationPath $DependencyDestinationPath -ResolveDependancies:$ResolveDependancies.IsPresent -ErrorAction Stop -ModuleValidationCache $ModuleValidationCache
									$ModDependencyFound = $true
								}
...
function Publish-PSModule
{
    ...
    Process
    {
		...
		try
		{
			...
			$m = Get-Module -Name PowerShellGet -Verbose:$false | Select-Object -Last 1
			$mSBAsString='function init_var {[cmdletbinding()] param () Install-NuGetClientBinaries -BootstrapNuGetExe -CallerPSCmdlet $PSCmdlet -force -verbose:$false}; init_var'
			$null = $m.Invoke([scriptblock]::Create($mSBAsString))
			$NugetExePath = $m.Invoke({$NuGetExePath})

			#Check if source is already added
			$NugetUserConfigFilePath = "$env:APPDATA\NuGet\NuGet.Config"
			if (test-path -Path $NugetUserConfigFilePath)
			{
				$Sources = Start-NewProcess -FilePath $NugetExePath -Arguments "sources list -configfile $NugetUserConfigFilePath" -ReturnResult -ErrorAction Stop
			}
			if ($Sources -ilike "*$($PSGetRepository['Name'])*")
			{
				$Action = 'update'
			}
			else
			...
			if ($PSGetRepository.ContainsKey('Credential'))
			{
				...
				$StartProcess_params.Add('ArgumentList',"sources $Action -name $($PSGetRepository['Name']) -source $($PSGetRepository['PublishLocation']) -user $UserName -pass $UserPassword -configfile $NugetUserConfigFilePath")
			}
			else
			{
		...
		foreach ($Repo in $PSGetRepository)
		{
			...
			try
			{
				...
				if ($RepoCheck)
				{
					...
					{
						Set-PSRepository -Name $Repo.Name -PublishLocation $Repo.PublishLocation
					}
